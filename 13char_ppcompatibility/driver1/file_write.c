...
// volatile long unsigned int *control_reg,status_reg;
 volatile unsigned char control_reg,status_reg;
...
size_t lsize;
	...
	for(l = 0;l < num_of_qset ; l++)
	{
		...
		outb(*(char *)dptr->data[0],DATAADD);

		outb(0x00,CONTROLADD);			//clear strobe
